...
class SeqMat(dict):
    ...
    def _init_zero(self):
        for i in self.ab_list:
            j_range = self.ab_list if (
                self.non_synon == True) else self.ab_list[:self.ab_list.index(i) + 1]
            for j in j_range:
                self[j, i] = 0.
    ...
    def make_full_mat(self):
        ...
        for key in temp_copy:
            try:
                # don't over-write the reverse keys if they are already
                # initialized
...
def _build_log_odds_mat(subs_mat, logbase=2, factor=10.0, round_digit=0, keep_nd=0):
    ...
    for key, value in subs_mat.items():
        if value < EPSILON:
            ...
            lo_mat[key] = round(
                factor * log(value) / log(logbase), round_digit)
...
def make_log_odds_matrix(acc_rep_mat, exp_freq_table=None, logbase=2,
    ...
    lo_mat = _build_log_odds_mat(
        subs_mat, logbase, factor, round_digit, keep_nd)
    return lo_mat
...
def two_mat_relative_entropy(mat_1, mat_2, logbase=2, diag=diagALL):
    ...
    if len(key_list_1) != len(key_list_2):
        sys.stderr.write(
            "Warning: first matrix has more entries than the second\n")
