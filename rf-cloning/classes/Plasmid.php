...
class Plasmid
{
	...
	private $insert_name, $backbone_name, $backbone_database, $backbone_id, $orig_plasmid_seq, $insert_seq, $insert_sites;
	private $fwd_primer, $rev_primer, $notes, $savvy_meta, $complete, $proj_hash;
	private $checksum;
	private $conn;
	...
	public function __toString() {
		$output = "plasmid_id:".$this->plasmid_id."<br />";
		$output .= "plasmid_name:".$this->plasmid_name."<br />";
		$output .= "user_id:".$this->user_id."<br />";
		$output .= "plasmid_seq:".$this->plasmid_seq."<br />";
		$output .= "savvy_markers:".$this->savvy_markers."<br />";
		$output .= "savvy_enzymes:".$this->savvy_enzymes."<br />";
		$output .= "popularity:".$this->popularity."<br />";
		$output .= "privacy:".$this->privacy."<br />";
		$output .= "insert_name:".$this->insert_name."<br />";
		$output .= "backbone_name:".$this->backbone_name."<br />";
		$output .= "backbone_database:".$this->backbone_database."<br />";
		$output .= "backbone_id:".$this->backbone_id."<br />";
		$output .= "orig_plasmid_seq:".$this->orig_plasmid_seq."<br />";
		$output .= "insert_seq:".$this->insert_seq."<br />";
		$output .= "insert_sites:".$this->insert_sites."<br />";
		$output .= "fwd_primer:".$this->fwd_primer."<br />";
		$output .= "rev_primer:".$this->rev_primer."<br />";
		$output .= "notes:".$this->notes."<br />";
		$output .= "savvy_meta:".$this->savvy_meta."<br />";
		$output .= "complete:".$this->complete."<br />";
		$output .= "proj_hash:".$this->proj_hash."<br />";
		$output .= "checksum:".$this->checksum."<br />";
		$output .= "database:".$this->database."<br />";
		return $output;
	}
	...
	public function get_database_plasmid($plasmid_id, $database)
	{
		...
		try
		{
			$plasmid_info_query = mysqli_query($this->conn, "SELECT * FROM ".$this->database." WHERE plasmid_id = ".$this->plasmid_id.";");
			if (mysqli_error($this->conn))
			{
				throw new Exception(mysqli_error($this->conn));
			}

			$plasmid_info_array = mysqli_fetch_assoc($plasmid_info_query);
		}
		...
		if($this->database == "projects" && $plasmid_info_array['backbone_database'] != "blank")
		{
			try
			{
				$plasmid_info = mysqli_fetch_row(mysqli_query($this->conn, "SELECT plasmid_name FROM ".$plasmid_info_array['backbone_database']." WHERE plasmid_id = ".$plasmid_info_array['backbone_id'].";"));
				if (mysqli_error($this->conn))
				{
					throw new Exception(mysqli_error($this->conn));
				}
	...
	private function clean_sequence($sequence)
	{
		...
		$sequence = preg_replace("/[^A-Z]/i","",$sequence);

		//convert any non-standard nucleotides into 'X'
		$sequence = preg_replace("/[^ATGC]/i","X",$sequence);
	...
	{
		$params_array = array("user_id" => $this->user_id,
			                  "plasmid_name" => $this->plasmid_name,
			                  "insert_name" => $this->insert_name,
			                  "backbone_id" => $this->backbone_id,
			                  "backbone_database" => $this->backbone_database,
			                  "sequence" => $this->plasmid_seq,
			                  "plasmid_sequence" => $this->orig_plasmid_seq,
			                  "insert_sequence" => $this->insert_seq,
			                  "insert_sites" => $this->insert_sites,
			                  "fwd_primer" => $this->fwd_primer,
			                  "rev_primer" => $this->rev_primer,
			                  "new_size" => strlen($this->plasmid_seq),
			                  "notes" => $this->notes,
			                  "savvy_markers" => $this->savvy_markers,
			                  "savvy_enzymes" => $this->savvy_enzymes,
			                  "savvy_meta" => $this->savvy_meta,
			                  "complete" => $this->complete,
			                  "checksum" => md5($this->plasmid_seq),
			                  "proj_hash" => $this->proj_hash);
		...
		$mysql_query = substr($mysql_query,0,-2);  // Strip trailing comma
		$mysql_query .= ";";
	...
	private function mysqlQuery($query_string, $origination)
	{
		try
		{
			mysqli_query($this->conn, $query_string);
			if (mysqli_error($this->conn))
			{
				throw new Exception(mysqli_error($this->conn));
			}
