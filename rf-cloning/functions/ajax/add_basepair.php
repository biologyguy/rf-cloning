...
switch($_POST['primer_dir_code'])
	{
	case 1: //fwd-fwd
		if (empty($split_fwd_primer_array[2]) && (strlen($_POST['insert_seq']) == strlen($split_fwd_primer_array[1])))
			{
			$split_fwd_primer_array[2] = "";	
			}
		
		if (count($split_fwd_primer_array) == 3 )
			{
			//attach the back end of the plasmid sequence to the front end, to make sure there isn't any problem getting the next basepair
			$plasmid_seq = $plasmid_array[2].$plasmid_array[0];				
			$final_sequence = substr($plasmid_seq,0,(strlen($split_fwd_primer_array[2])+1));		
			$fwd_primer_database = $split_fwd_primer_array[0]."|".$split_fwd_primer_array[1]."|".$final_sequence;		
			}
		
		else
			{
			$final_sequence = substr($_POST['insert_seq'],0,strlen($split_fwd_primer_array[1])+1);		
	...
	case 2: //fwd-rev
		...
		//Reverse the plasmid sequence to make it easier to get the primer.
		$rev_plas = strrev($plasmid_seq);
		$final_sequence = strrev(substr($rev_plas,0,strlen($split_fwd_primer_array[0])+1));
	...
	case 3: //rev-fwd
		if (empty($split_rev_primer_array[2]) && (strlen($_POST['insert_seq']) == strlen($split_rev_primer_array[1])))
			{
			$split_rev_primer_array[2] = "";	
			}
		
		if (count($split_rev_primer_array) == 3 )
			{
			//attach the back end of the plasmid sequence to the front end, to make sure there isn't any problem getting the next basepair
			$plasmid_seq = rev_comp($plasmid_array[2].$plasmid_array[0]);				
			$final_sequence = substr($plasmid_seq,0,(strlen($split_rev_primer_array[2])+1));		
			$rev_primer_database = $split_rev_primer_array[0]."|".$split_rev_primer_array[1]."|".$final_sequence;			
			}
		
		else
			{
			$final_sequence = substr(rev_comp($_POST['insert_seq']),0,strlen($split_rev_primer_array[1])+1);		
	...
	case 4: //rev-rev
		...
		//Reverse the plasmid sequence to make it easier to get the primer.
		$rev_plas = strrev(rev_comp($plasmid_seq));
		$final_sequence = strrev(substr($rev_plas,0,strlen($split_rev_primer_array[0])+1));
